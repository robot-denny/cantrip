...
Most of this runbook is mechanical, and identical on every project that will ever run `/guide`. The
rest is yours. The division follows one rule: *conform where it shows, standardize where it does
not*. Editors and visitors see your markup, so markup should conform to your project conventions.
Nobody ever sees a document type alias, so standardizing those lets one shared reference describe
them for everyone.
...
an alias, so if one of these names is already taken in your project, pick another. Make a note of
which one, because you'll record it in Step 8. The config records what a project **used**, never what
...
replacement, and nothing overwrites it. If a row ever has to be removed while carrying one of those
notes, you'll be asked first rather than told afterwards.

Notice that the rows are flat. Each one carries its own `tab` and `group` as ordinary text rather
than sitting inside nested containers. That is what lets a field move from one tab to another as a
...
Umbraco creates `editorGuide.cshtml` and names it for you, and you'll fill it in at Step 5.
...
**Leave every one of these optional. Don't mark anything mandatory.** A required field here breaks
three things that all need to work:
...
property that can store and hand back a small piece of JSON. A Textarea does the job, and the toolkit
...
instance of the block, sitting in the page and rendered by your site like any other content. It is
not a screenshot or generated artwork.
...
Don't give it a template. Its own URL will return a 404, which is fine. It's a shelf rather than a
page. The kinds themselves are `element` for a block's guide and `document` for a page type's.
...
There's deliberately nowhere on this type to store a list of guides. A stored list is a second copy
of the content tree, and it goes wrong the moment anyone adds, renames, unpublishes or reorders a
guide. You'd have to run something to put it right, and in between it would be wrong without saying
so. Worked out at render time instead, publishing a guide is all it takes to add it to the index.
...
**Render the alt text on your screenshots.** A screenshot in a guide is doing real work. It is often
the only place an editor will ever see a particular settings screen. So make alt text required on the
media type and put it on the page. Nothing in this toolkit creates media or could write those words
for you, and the person uploading the image is the last one who knows what it shows.
...
1. **Read only three things from each guide: its name, its URL, and its blurb-or-purpose line.** Not
   the property rows, and not the live examples. The obvious approach, asking each guide for its
   content and taking a teaser from it, pulls in everything that guide holds. Forty guides with
   twenty rows each means loading **eight hundred table rows, plus every live example**, just to
   print forty names and forty sentences. Kept to three values, it's one cheap read per guide.
...
changeable forever. An editor can rename this node next week. Someone can move it when the tree gets
reorganized. Nothing breaks, because the toolkit finds it by its stored key rather than by its route
or its name. If you'd rather it didn't show up in the main navigation, use `umbracoNaviHide` or
...
node. Writing editor-facing pages into an unrelated corner of your content tree is far worse than
finding nothing at all, because finding nothing is something it can tell you about.

### Step 9 — Serialize the schema, and check the other slot

Export your new document types however your project normally does it, with uSync or Umbraco Deploy,
and commit them.
...
but it can never confirm that nothing has changed. Every later run will then offer to regenerate
rather than telling you the guide is still current.
...
- It fills the seeded fields **only when the page is created**: the purpose sentence, the when-to-use
  section, and the live examples. **Skip one of those now and you won't be offered it again.** Give
  them a proper look rather than waving them through.
